      ...
      <template #tableTitle>
        <div class="w-1/1 flex items-center">
          ...
          <div class="flex items-center ml-4">
            <span>班级：</span>
            <div>
              <el-select v-model="class_ids" multiple placeholder="请选择" filterable @change="getCount">
...
import { getSigninCount, getCourseListTeach } from './signinCount.api'
import { ref, reactive, nextTick, computed } from 'vue'
import BasicTable from '/@/components/Table/BasicTable.vue'
import { columns } from './signinCount.data'
...
const total = ref(0)
const dataSource: SignInInfo[] = reactive([])
...
const title = computed(() => {
  const class_name = classOption
    .filter(item => class_ids.value.includes(item.value))
    .map(item => item.label)
    .join(',')
  const course_name = courseOption.filter(item => item.value === course_id.value)[0]?.label
  return class_name + '——' + course_name + '考勤数据可视化'
})
...
const toShow = () => {
  if (!course_id.value || !class_ids.value.length) {
...
const toExport = () => {
  ...
  const headers = ['课程编号', '课程名称', '班级编号', '班级', '姓名', '日期', '课时']
  exportExcel(
    dataSource.map(item => {
      return {
...
function getCount() {
  ...
  const postData = {
    ...
    class_ids: class_ids.value.join(',')
  }
  getSigninCount(postData).then((res: any) => {
